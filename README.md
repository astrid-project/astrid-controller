...
Docker image has to be built in a local repository
`docker-compose build`

### Deployment in k8s
configMap named `astrid` should be created. Example file should be similar to file `configMap.yaml`. Here IP address of the ContextBroker will be provided.
* `kubectl create configmap astrid --from-env-file=configMap.yaml -o yaml --dry-run | kubectl replace -f -`
